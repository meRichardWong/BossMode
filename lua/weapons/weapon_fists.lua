...
AddCSLuaFile('weapon_fists.lua')

SWEP.PrintName = "Ground Pound"
SWEP.Author = "richard"
SWEP.Purpose = "pound your friends"
...
function SWEP:Deploy()
	...
	self.Owner:SetGravity(2)
	self.Owner:SetJumpPower(1300)
...
function PlayerLanded(player, inWater, onFloater, speed )

	local ThornCount = 10
	
	if (SERVER && player:GetActiveWeapon():GetClass() == "weapon_ground_pound") then 
		...
		local NormalizedThornVector = ThornLine:GetNormalized()

		print( player:GetPos():Distance(ThornLine))
		
		player:GodEnable()
		
		for i=0, ThornCount do
			local ThornPosition = player:GetPos() + NormalizedThornVector * i * 50
			ThornPosition.z = player:GetPos().z

			local explode = ents.Create("env_explosion") -- creates the explosion
			explode:SetPos(ThornPosition)
